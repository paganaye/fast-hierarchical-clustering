...
describe("A Quad", function() {
  ...
  it("has a recursive structure", function() {
    ...
    expect(quad.root.topLeft?.topLeft?.points[0].x).to.eq(0);
  });
  it("has variable precision", function() {
    ...
    expect(quad.root.topLeft?.topLeft?.topLeft?.points[0].x).to.eq(0);
  });
  it("can be trimmed", function() {
    ...
    expect(quad.root.topLeft?.topLeft?.points[0].x).to.eq(0);
    quad.trim();
    expect(quad.root.topLeft?.points[0].x).to.eq(0);
    quad.trim();
    expect(quad.root.points[0].x).to.eq(0);
  });
