...
describe('Faster Avg algorithm', function () {
  ...
  it('quintet 1497e39', function () {
    let a = new Point(0.9999, 0.8797, "A");
    let b = new Point(0.1549, 0.6318, "B");
    let c = new Point(0.9855, 0.5666, "C");
    let d = new Point(0.3062, 0.8455, "D");
    let e = new Point(0.5890, 0.7953, "E");
    // aa 0.000 ab 0.881 ac 0.313 ad 0.695 ae 0.420
    // ba 0.881 bb 0.000 bc 0.833 bd 0.262 be 0.464
    // ca 0.313 cb 0.833 cc 0.000 cd 0.734 ce 0.458

    // da 0.695 db 0.262 dc 0.734 dd 0.000 de 0.287
    // ea 0.420 eb 0.464 ec 0.458 ed 0.287 ee 0.000
    let points = [a, b, c, d, e];
    let classicAlgorithm = getNormalizedDendrogram(new ClassicAvgAlgorithm(points));
    let newAlgorithm = getNormalizedDendrogram(new FasterAvgAlgorithm(4, points));
    expect(classicAlgorithm).to.eq("((A-C)-(E-(B-D)))");
    expect(newAlgorithm).to.eq("((A-C)-(E-(B-D)))");
    // and not (((A-C)-E)-(B-D))
  });
});
